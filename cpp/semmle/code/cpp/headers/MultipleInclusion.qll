 ...
 */
deprecated
class NotIncludedGuard extends IncludeGuardedHeader {
  NotIncludedGuard() {
    none()
  }
...
pragma[noopt] predicate correctIncludeGuard(HeaderFile hf, PreprocessorDirective ifndef, Macro define, PreprocessorEndif endif, string includeGuard) {
  ...
  not exists(int relevant,
               Location ifndefLocation, int ifndefLine,
               Location defineLocation, int defineLine |
...
predicate ifndefDirective(PreprocessorDirective ppd, string macro) {
  ...
  (ppd instanceof PreprocessorIf and
   exists(string head | head = ppd.getHead() |
     macro = head.replaceAll("(", " ").replaceAll(")", "").replaceAll("\t", " ").regexpCapture("[ ]*![ ]*defined[ ]+([^ ]*)[ ]*", 1).trim()
   ))
}
