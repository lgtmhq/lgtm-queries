...
class MetricFile extends File {
   ...
   */
   ...
   private string getAUsedHalsteadN1Operator() {
     exists(CommaExpr e | e.getFile() = this) and result = "comma" or
     exists(ReferenceToExpr e | e.getFile() = this) and result = "refTo" or
     exists(PointerDereferenceExpr e | e.getFile() = this) and result = "dereference" or
     exists(CStyleCast e | e.getFile() = this) and result = "cCast" or
     exists(StaticCast e | e.getFile() = this) and result = "staticCast" or
     exists(ConstCast e | e.getFile() = this) and result = "constCast" or
     exists(ReinterpretCast e | e.getFile() = this) and result = "reinterpretCast" or
     exists(DynamicCast e | e.getFile() = this) and result = "dynamicCast" or
     exists(SizeofExprOperator e | e.getFile() = this) and result = "sizeofExpr" or
     exists(SizeofTypeOperator e | e.getFile() = this) and result = "sizeofType" or
     exists(IfStmt e | e.getFile() = this) and result = "ifVal" or
     exists(SwitchStmt e | e.getFile() = this) and result = "switchVal" or
     exists(ForStmt e | e.getFile() = this) and result = "forVal" or
     exists(DoStmt e | e.getFile() = this) and result = "doVal" or
     exists(WhileStmt e | e.getFile() = this) and result = "whileVal" or
     exists(GotoStmt e | e.getFile() = this) and result = "gotoVal" or
     exists(ContinueStmt e | e.getFile() = this) and result = "continueVal" or
     exists(BreakStmt e | e.getFile() = this) and result = "breakVal" or
     exists(ReturnStmt e | e.getFile() = this) and result = "returnVal" or
     exists(SwitchCase e | e.getFile() = this) and result = "caseVal" or
     exists(IfStmt s | s.getFile() = this and s.hasElse()) and result = "elseVal" or
     exists(Function f | f.getFile() = this) and result = "function" or
     exists(Class c | c.getFile() = this) and result = "classDef" or
     exists(TypeDeclarationEntry e | e.getFile() = this) and result = "typeDecl" or
...
private predicate aClassFile(Class c, File file)
{
...
predicate dependsOnFileSimple(MetricFile source, MetricFile dest) {
     ...
     or exists(FunctionCall ca, Function f |
       ca instanceof FunctionCall and
       ca.getFile() = source and ca.getTarget() = f and f.getFile() = dest
       and not f.isMultiplyDefined()
       and not exists(Function ef | ef = ca.getEnclosingFunction() and ef.isMultiplyDefined())
     )
     or exists(Access a, Declaration d |
       a instanceof Access and
       a.getFile() = source and a.getTarget() = d and d.getFile() = dest
       and not exists(Function ef | ef = a.getEnclosingFunction() and ef.isMultiplyDefined())
     )
     or exists(Variable v, VariableDeclarationEntry e, Type vt, UserType t |
       e instanceof VariableDeclarationEntry and
       e.getFile() = source and v.getADeclarationEntry() = e and vt = v.getType() and
       vt.refersTo(t) and t instanceof UserType and t.getFile() = dest
     )
     or exists(Function f, FunctionDeclarationEntry e, Type ft, UserType t |
       e instanceof FunctionDeclarationEntry and
       e.getFile() = source and f.getADeclarationEntry() = e and ft = f.getType() and
       ft.refersTo(t) and t instanceof UserType and t.getFile() = dest
     )
     or exists(MacroInvocation mi, Macro m |
       mi instanceof MacroInvocation and
       mi.getFile() = source and mi.getMacro() = m and
       m.getFile() = dest
     )

     or exists(TypedefType t, TypeDeclarationEntry e, Type bt, UserType u |
       e instanceof TypeDeclarationEntry and
       e.getFile() = source and t.getADeclarationEntry() = e and  bt = t.getBaseType() and
       bt.refersTo(u) and u instanceof UserType and u.getFile() = dest
     )
     or exists(Cast c, Type t, UserType u |
       c instanceof Cast and
       c.getFile() = source and c.getType() = t and
       t.refersTo(u) and u instanceof UserType and u.getFile() = dest
       and not exists(Function ef | ef = c.getEnclosingFunction() and ef.isMultiplyDefined())
     )
  }
