 ...
 * member.
 */
private cached ControlFlowNode getControlFlowEntry(ControlFlowNode node) {
 ...
 */
private cached ControlFlowNode mostRecentSideEffect(ControlFlowNode node) {
...
private cached newtype GVNBase =
  GVN_IntConst(int val, Type t) { mk_IntConst(val,t,_) }
  or
  GVN_FloatConst(float val, Type t) { mk_FloatConst(val,t,_) }
  or
  ...
  GVN_OtherVariable(Variable x, ControlFlowNode dominator) {
     mk_OtherVariable(x, dominator, _)
  }
  or
  ...
  or
  GVN_Conversion(Type t, GVN child) { mk_Conversion(t, child, _) }
  or
  GVN_BinaryOp(GVN lhs, GVN rhs, string opname) {
    mk_BinaryOp(lhs, rhs, opname, _)
  }
  or
  GVN_UnaryOp(GVN child, string opname) { mk_UnaryOp(child, opname, _) }
  or
  GVN_ArrayAccess(GVN x, GVN i, ControlFlowNode dominator) {
...
class GVN extends GVNBase {
  ...
  Expr getAnExpr() {
    this = globalValueNumber(result)
  }

  /** Gets the kind of the GVN. This can be useful for debugging. */
  string getKind() {
    if this instanceof GVN_IntConst then result = "IntConst" else
    if this instanceof GVN_FloatConst then result = "FloatConst" else
    if this instanceof GVN_UndefinedLocalScopeVariable then
      result = "UndefinedLocalScopeVariable" else
    if this instanceof GVN_OtherVariable then result = "OtherVariable" else
    if this instanceof GVN_FieldAccess then result = "FieldAccess" else
    if this instanceof GVN_Deref then result = "Deref" else
    if this instanceof GVN_ThisExpr then result = "ThisExpr" else
    if this instanceof GVN_Conversion then result = "Conversion" else
    if this instanceof GVN_BinaryOp then result = "BinaryOp" else
    if this instanceof GVN_UnaryOp then result = "UnaryOp" else
    if this instanceof GVN_ArrayAccess then result = "ArrayAccess" else
    if this instanceof GVN_Unanalyzable then result = "Unanalyzable" else
    result = "error"
  }
...
private predicate mk_DotFieldAccess(
  GVN qualifier, Field target, DotFieldAccess access) {
...
private predicate mk_PointerFieldAccess(
  GVN qualifier, Field target, ControlFlowNode dominator,
...
private predicate mk_ImplicitThisFieldAccess(
  Function fcn, Field target, ControlFlowNode dominator,
  ImplicitThisFieldAccess access) {
...
private predicate mk_ImplicitThisFieldAccess_with_deref(
  ...
  | mk_ImplicitThisFieldAccess_with_qualifier(
      qualifier, target, dominator, access) and
    new_qualifier = GVN_Deref(qualifier, dominator))
}
...
private predicate mk_OtherVariable(
  Variable x, ControlFlowNode dominator, VariableAccess access) {
...
private predicate mk_BinaryOp(
  GVN lhs, GVN rhs, string opname, BinaryOperation op) {
...
private predicate mk_Deref(
  GVN p, ControlFlowNode dominator, PointerDereferenceExpr deref) {
...
cached GVN globalValueNumber(Expr e) {
