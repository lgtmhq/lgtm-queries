 ...
 */
private
float wideningLowerBounds(ArithmeticType t) {
  result = 2.0 or
  result = 1.0 or
  result = 0.0 or
  result = -1.0 or
...
private
float wideningUpperBounds(ArithmeticType t) {
  result = -2.0 or
  result = -1.0 or
  result = 0.0 or
  result = 1.0 or
 ...
 */
private
predicate defDependsOnDef(
  RangeSsaDefinition def, LocalScopeVariable v,
  RangeSsaDefinition srcDef, LocalScopeVariable srcVar
) {
  // Definitions with a defining value.
  exists (Expr expr
  | assignmentDef(def, v, expr)
...
predicate exprDependsOnDef(
  Expr e, RangeSsaDefinition srcDef, LocalScopeVariable srcVar
) {
  exists (UnaryMinusExpr negateExpr
  | e = negateExpr
  ...
  | e = minExpr
  | exprDependsOnDef(minExpr.getAnOperand(), srcDef, srcVar))
  or
  exists (MaxExpr maxExpr
  | e = maxExpr
  | exprDependsOnDef(maxExpr.getAnOperand(), srcDef, srcVar))
  or
 ...
 */
private
predicate phiDependsOnDef(
  RangeSsaDefinition phi, LocalScopeVariable v,
  RangeSsaDefinition srcDef, LocalScopeVariable srcVar
) {
...
private
predicate defDependsOnDefTransitively(
  RangeSsaDefinition def, LocalScopeVariable v,
  RangeSsaDefinition srcDef, LocalScopeVariable srcVar
) {
  defDependsOnDef(def, v, srcDef, srcVar) or
...
private
predicate isRecursiveDef(RangeSsaDefinition def, LocalScopeVariable v) {
...
private
predicate analyzableDef(RangeSsaDefinition def, LocalScopeVariable v) {
 ...
 */
private
float getTruncatedLowerBounds(Expr expr) {
  if analyzableExpr(expr)
     then // If the expression evaluates to a constant, then there is no
 ...
 */
deprecated predicate negative_overflow(Expr expr) {
...
private predicate exprIsUsedAsBool(Expr expr) {
  expr = any(BinaryLogicalOperation op).getAnOperand().getFullyConverted() or
 ...
 */
private
float getPhiLowerBounds(LocalScopeVariable v, RangeSsaDefinition phi) {
  exists (
    VariableAccess access, ComparisonOperation guard, boolean branch,
    float defLB, float guardLB
  | access = v.getAnAccess() and
...
private
float getPhiUpperBounds(LocalScopeVariable v, RangeSsaDefinition phi) {
  exists (
    VariableAccess access, ComparisonOperation guard, boolean branch,
    float defUB, float guardUB
  | access = v.getAnAccess() and
    phi.isGuardPhi(access, guard, branch) and
...
private
float getDefLowerBoundsImpl(RangeSsaDefinition def, LocalScopeVariable v) {
  // Definitions with a defining value.
  exists (Expr expr
  | assignmentDef(def, v, expr)
...
private
float getDefUpperBoundsImpl(RangeSsaDefinition def, LocalScopeVariable v) {
  // Definitions with a defining value.
  exists (Expr expr
  | assignmentDef(def, v, expr)
  | result = getFullyConvertedUpperBounds(expr))
  or
  exists (
 ...
 */
private
float getDefLowerBounds(RangeSsaDefinition def, LocalScopeVariable v) {
  ...
  | // Widening: check whether the new lower bound is from a source which
    ...
    if isRecursiveDef(def, v)
      then // The new lower bound is from a recursive source, so we round
...
private
float getDefUpperBounds(RangeSsaDefinition def, LocalScopeVariable v) {
  ...
  | // Widening: check whether the new upper bound is from a source which
    // depends recursively on the current definition.
    if isRecursiveDef(def, v)
      then // The new upper bound is from a recursive source, so we round
 ...
 */
private
predicate unanalyzableDefBounds(
 ...
 */
private
predicate boundFromGuard(
  ComparisonOperation guard, VariableAccess v,
  float boundValue,
  boolean isLowerBound, RelationStrictness strictness, boolean branch
) {
  exists (
    float p, float q, float r, boolean isLB
  | linearBoundFromGuard(
      guard, v, p, q, r, isLB, strictness, branch) and
    boundValue = (r - q)/p
  | // If the multiplier is negative then the direction of the comparison
    // needs to be flipped.
    (p > 0 and isLowerBound = isLB) or
 ...
 */
private
predicate linearBoundFromGuard(
  ComparisonOperation guard, VariableAccess v,
  float p, float q, float boundValue,
  boolean isLowerBound,  // Is this a lower or an upper bound?
  RelationStrictness strictness,
...
private
predicate getBounds(Expr expr, float boundValue, boolean isLowerBound) {
  (isLowerBound = true and boundValue = getFullyConvertedLowerBounds(expr)) or
  (isLowerBound = false and boundValue = getFullyConvertedUpperBounds(expr))
}

/** Utility for `linearBoundFromGuard`. */
private
predicate exprTypeBounds(Expr expr, float boundValue, boolean isLowerBound) {
  (isLowerBound = true and boundValue = exprMinVal(expr.getFullyConverted())) or
