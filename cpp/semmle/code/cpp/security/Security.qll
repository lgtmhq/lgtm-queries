...
class SecurityOptions extends string {
  ...
  predicate sqlArgument(string function, int arg) {
    // MySQL C API
    (function = "mysql_query" and arg = 1) or
    (function = "mysql_real_query" and arg = 1) or
  ...
  predicate userInputArgument(FunctionCall functionCall, int arg)
  {
    exists(string fname |
      ...
      (
        (fname = "read" and arg = 1) or
        (fname = "fread" and arg = 0) or
        (fname = "fgets" and arg = 0) or
        (fname = "fgetws" and arg = 0) or
        (fname = "gets" and arg = 0) or
        (fname = "getaddrinfo" and arg = 3) or
        (fname = "recv" and arg = 1) or
        (fname = "recvfrom" and (arg = 1 or arg = 4 or arg = 5)) or
        (fname = "recvmsg" and arg = 1) or
        (fname = "scanf" and arg >= 1) or
   ...
   */
  predicate userInputReturn(string function)
  {
    none()
  }
  ...
  predicate isProcessOperationArgument(string function, int arg) {
    ...
    or (function = "popen" and arg = 0)
    or (function = "execl" and arg = 0)
    or (function = "execlp" and arg = 0)
    or (function = "execle" and arg = 0)
    or (function = "execv" and arg = 0)
    or (function = "execvp" and arg = 0)
    or (function = "execvpe" and arg = 0)
    or (function = "dlopen" and arg = 0)

    // Windows
    or (function = "LoadLibrary" and arg = 0)
    or (function = "LoadLibraryA" and arg = 0)
    or (function = "LoadLibraryW" and arg = 0)
  }
  ...
  predicate isUserInput(Expr expr, string cause) {
    exists(FunctionCall fc, int i |
      this.userInputArgument(fc, i)
      and expr = fc.getArgument(i)
      and cause = fc.getTarget().getName())
    or exists(FunctionCall fc |
      this.userInputReturned(fc)
      and expr = fc
      and cause = fc.getTarget().getName())
    or (commandLineArg(expr) and cause = "argv")
    or expr.(EnvironmentRead).getSourceDescription() = cause
  }
  ...
  predicate raisesPrivilege(Expr expr) {
    exists (ReturnStmt ret | ret.getExpr() = expr |
      ret.getEnclosingFunction().getName() = "checkPinCode"
      and ret.getExpr().getValue() = "1")
...
predicate argv(Parameter argv)
{
  exists(Function f |
    ...
    and f.getParameter(1) = argv)
}

/** Convenience accessor for SecurityOptions.isPureFunction */
predicate isPureFunction(string name) {
  exists (SecurityOptions opts | opts.isPureFunction(name))
}   
...
predicate raisesPrivilege(Expr expr) {
  exists (SecurityOptions opts | opts.raisesPrivilege(expr))
}
