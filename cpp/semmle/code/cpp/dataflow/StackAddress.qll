...
predicate stackPointerFlowsToUse(
  Expr use, Type useType, Expr source, boolean isLocal) {
  ...
  stackReferenceFlowsToUse(
    use.(AddressOfExpr).getOperand(), useType, source, isLocal)
  or
...
predicate stackReferenceFlowsToUse(
  ...
  // stackPointerFlowsToUse.
  stackPointerFlowsToUse(
    use.(ArrayExpr).getArrayBase(), useType, source, isLocal)
  or
  // Pointer dereference: *x
  stackPointerFlowsToUse(
    use.(PointerDereferenceExpr).getOperand(), useType, source, isLocal)
  or
...
predicate stackPointerFlowsToDef(
  SsaDefinition def, LocalScopeVariable var,
  Type useType, Expr source, boolean isLocal) {
  stackPointerFlowsToUse(
    def.getDefiningValue(var), useType, source, isLocal)
  or
  ...
  stackPointerFlowsToDef(
    def.getAPhiInput(var), var, useType, source, isLocal)
}
...
predicate pointerParamFlowsToDef(
  SsaDefinition def, Parameter param, Type useType) {
  ...
  | call.getTarget() = param.getFunction() and
    stackPointerFlowsToUse(
      call.getArgument(param.getIndex()), useType, _, _) and
