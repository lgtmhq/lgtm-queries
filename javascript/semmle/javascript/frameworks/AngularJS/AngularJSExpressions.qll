 ...
 */
private string getInterpolatedExpressionPattern() {
  result = "(?<=\\{\\{).*?(?=\\}\\})"
}
 ...
 */
abstract class NgToken extends TNgToken { // (only exposed for testability)
  ...
  private int getIndex() {
    exists (NgSource src, int start | this.at(src, start) |
      start = rank[result+1](NgToken someToken, int someStart | someToken.at(src, someStart) | someStart)
    )
  }
...
private module Lexer {
   ...
   */
  private abstract class NgTokenType extends string {
    bindingset[this] NgTokenType() { any() }
  ...
  private abstract class NgInternalToken extends TNgToken, NgToken {

    override predicate at(NgSource src, int start) {
      this = MkNgToken(src, start, _, _)
    }
    ...
    override predicate is(string content){
      this = MkNgToken(_, _, _, content)
    }
  ...
  class NgStringToken extends NgInternalToken {
    NgStringToken() {
      this = MkNgToken(_, _, any(NgStringTokenType t), _)
    }
  }
  ...
  class NgNumToken extends NgInternalToken {
    NgNumToken() {
      this = MkNgToken(_, _, any(NgNumberTokenType t), _)
    }
  }
  ...
  class NgIdentToken extends NgInternalToken {
    NgIdentToken() {
      this = MkNgToken(_, _, any(NgIdentTokenType t), _)
    }
  }
  ...
  class NgOtherToken extends NgInternalToken {
    NgOtherToken() {
      this = MkNgToken(_, _, any(NgOtherTokenType t), _)
    }
  }
  ...
  class NgOpToken extends NgInternalToken {
    NgOpToken() {
      this = MkNgToken(_, _, any(NgOpTokenType t), _)
    }
  }
}
...
abstract class NgAstNode extends TNode {
   ...
   */
  language[monotonicAggregates] string ppChildren() {
...
class NgAst extends TNgAst, NgAstNode {
  ...
  override NgAstNode getChild(int n) {
    n = 0 and this = TNgAst(_, _, _, result)
  }
...
class NgExprStmt extends TNgExprStmt, NgAstNode {
  override predicate at(NgToken start, NgToken end) {
    this = TNgExprStmt(start, end, _)
  }

  override string pp() {
    result = "(NgExprStmt: " + ppChildren() + ")"
  }

  override NgAstNode getChild(int n) {
    n = 0 and this = TNgExprStmt(_, _, result)
  }
}
...
class NgFilterChain extends TNgFilterChain, NgAstNode {
  override predicate at(NgToken start, NgToken end) {
    this = TNgFilterChain(start, end, _, _)
  }

  override string pp() {
    result = "(NgFilterChain: " + ppChildren() + ")"
  }
...
class NgVarExpr extends TNgVarExpr, NgExpr {
  ...
  NgVarExpr() {
    this = TNgVarExpr(identifier)
  }

  override predicate at(NgToken start, NgToken end) {
    start = end and start = identifier
  }

  override string pp() {
    result = "(NgVarExpr: " + getName() + ")"
  }
...
class NgDotExpr extends TNgDotExpr, NgExpr {
  override predicate at(NgToken start, NgToken end) {
    this = TNgDotExpr(start, end, _, _)
  }

  override string pp() {
    result = "(NgDotExpr: " + getBase().pp() + "." + getName() + ")"
  }

  override NgAstNode getChild(int n) {
    n = 0 and result = getBase()
  }
...
class NgCallExpr extends TNgCallExpr, NgExpr {
  override predicate at(NgToken start, NgToken end) {
    this = TNgCallExpr(start, end, _, _)
  }

  override string pp() {
    result = "(NgCallExpr: " + ppChildren() + ")"
  }

  override NgAstNode getChild(int n) {
    (n = 0 and this = TNgCallExpr(_, _, result, _)) or
...
class NgString extends TNgString, NgExpr {
  ...
  NgString() {
    this = TNgString(stringToken)
  }

  override predicate at(NgToken start, NgToken end) {
    start = end and start = stringToken
  }
...
class NgNumber extends TNgNumber, NgExpr {
  ...
  override predicate at(NgToken start, NgToken end) {
    start = end and start = numberToken
  }
  override string pp() {
    result = getValue()
  }
...
class NgFilterArgument extends TNgFilterArgument, NgMaybeFilterArgument {
  override predicate at(NgToken start, NgToken end) {
    this = TNgFilterArgument(start, end, _, _)
  }

  override string pp() {
    result = "(NgFilterArgument: " + ppChildren() + ")"
  }
...
class NgConsCallArgument extends TNgConsCallArgument, NgCallArguments {
  override predicate at(NgToken start, NgToken end) {
    this = TNgConsCallArgument(start, end, _, _)
  }

  override string pp() {
    result = "(NgConsCallArgument: " + ppChildren() + ")"
  }

  override NgAstNode getChild(int n) {
    (n = 0 and this = TNgConsCallArgument(_, _, result, _)) or
...
private module Parser {
  ...
  private NgMaybeFilter maybeFilter(NgToken start, NgToken end) {
    if start.succ().(NgOpToken).is("|") then
    result.(NgFilter).at(start.succ().succ(), end) else
    result.(Empty).at(start, end)
  }
  ...
  private NgMaybeFilterArgument maybeFilterArgument(NgToken start, NgToken end) {
    if start.succ().(NgOtherToken).is(":") then
    result.(NgFilterArgument).at(start.succ().succ(), end) else
    result.(Empty).at(start, end)
  }
  ...
  private NgCallArguments maybeFirstCallArgument(NgToken start, NgToken end) {
    if start.succ().(NgOtherToken).is(")") then
    result.(Empty).at(start, end) else
    result.(NgConsCallArgument).at(start.succ(), end)
  }
  ...
  private NgCallArguments maybeCallArgument(NgToken start, NgToken end) {
    if start.succ().(NgOtherToken).is(",") then
    result.(NgConsCallArgument).at(start.succ().succ(), end) else
    result.(Empty).at(start, end)
  }
  ...
  TNgAst(NgToken start, NgToken end, boolean oneTime, NgExprStmt stmt) {
    ...
    exists(NgToken stmtStart |
      if (start.(NgOtherToken).is(":") and start.succ().(NgOtherToken).is(":")) then
      (stmtStart = start.succ().succ() and oneTime = true) else
      (stmtStart = start and oneTime = false) |
      stmt.at(stmtStart, end)
    )
  } or
  TNgExprStmt(NgToken start, NgToken end, NgFilterChain c) {
    c.at(start, end)
  } or
