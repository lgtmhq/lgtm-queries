...
class CryptographicKeyCredentialsExpr extends CredentialsExpr {
  ...
  CryptographicKeyCredentialsExpr() {
    this = any(CryptographicKey k).asExpr()
  }
...
private module BrowserIdCrypto {
  ...
  private class Apply extends CryptographicOperation {
    ...
    DataFlow::Node getKey() {
      result.asExpr() = mce.getArgument(1)
    }
  }
...
private module NodeJSCrypto  {
  ...
  private class Apply extends CryptographicOperation, MethodCallExpr {
    ...
    override CryptographicAlgorithm getAlgorithm() {
      result = instantiation.getAlgorithm()
    }
  ...
  private class Key extends CryptographicKey {
    ...
    Key() {
      ...
      exists(DataFlow::SourceNode mod, string name, DataFlow::InvokeNode call, int index |
        ...
        index = 0 and (name = "privateDecrypt" or name = "privateEncrypt")
        or
        index = 1 and (name = "createCipheriv" or name = "Decipheriv" or name = "createHmac")
      )
    }
...
private module HashJs {
  ...
  private DataFlow::CallNode getAlgorithmExpr(CryptographicAlgorithm algorithm) {
    exists(string algorithmName | algorithm.matchesName(algorithmName) |
      ...
      or
      exists(DataFlow::SourceNode mod |
        mod = DataFlow::moduleImport("hash.js/lib/hash/" + algorithmName) or
        exists(string size |
...
private module Forge  {
  ...
  private abstract class Cipher extends DataFlow::CallNode {
  ...
  private class NonKeyCipher extends Cipher {
    ...
    NonKeyCipher() {
      exists (string algorithmName |
        ...
        this = getAnImportNode().getAPropertyRead("md").getAPropertyRead(algorithmName).getAMemberCall("create")
      )
    }
  ...
  private class Key extends CryptographicKey {
    ...
    Key() {
      this = any(KeyCipher cipher).getKey()
    }
...
private module Hasha  {
  ...
  module ExpressJwt {
    ...
    private class Key extends CryptographicKey {
      ...
      Key() {
        this = DataFlow::moduleMember("express-jwt", "sign").getACall().getArgument(1)
      }
