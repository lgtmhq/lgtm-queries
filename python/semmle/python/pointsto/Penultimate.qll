...
private predicate penultimate_from_import_points_to(ImportMemberNode f, Object value, ClassObject cls, ControlFlowNode origin) {
    ...
    exists(ImportMember im, string name |
        ...
        exists(ModuleObject module, ObjectOrCfg origin_or_obj |
            penultimate_points_to(f.getModule(name), module, _, _) and
            penultimate_module_attribute_points_to(module, name, value, cls, origin_or_obj) |
        ...
        not exists(ModuleObject module | /* 3. */
            ...
            layer0_module_attribute_points_to(module, name, _, _, _)
...
predicate penultimate_super_method_call(CallNode call, ClassObject self_type, FunctionObject method) {
    exists(AttrNode attr |
        ...
        exists(CallNode super_call, ClassObject start_type, string name |
            penultimate_super_call_types(super_call, self_type, start_type) |
...
ClassObject penultimate_class_base_type(ClassObject cls, int n) {
...
predicate penultimate_six_add_metaclass(CallNode decorator_call, ClassObject decorated, ControlFlowNode metaclass) {
    exists(CallNode decorator, FunctionObject six_meta |
        ...
        penultimate_points_to(decorator.getFunction(), six_meta, _, _) and
        decorator.getArg(0) = metaclass and
        exists(ModuleObject six |
            six.getName() = "six" and
            penultimate_module_attribute_points_to(six, "add_metaclass", six_meta, _, _)
        )
    )
}
...
predicate penultimate_failed_inference(ClassObject cls, string reason) {
    ...
    or
    exists(int i | exists(((ClassExpr)cls.getOrigin()).getBase(i)) and not exists(layer0_class_base_type(cls, i)) and reason = "Missing base " + i)
    or
    ...
    extensional_name(name) and (
      ...
      exists(ModuleObject module |
          ...
          origin = module.getModule().getEntryNode() and
           obj = module and
          penultimate_explicitly_imported(module) and
          cls = theModuleType()
      )
    )
}
...
private predicate penultimate_explicitly_imported(ModuleObject module) {
    exists(ImportExpr ie | penultimate_module_imported_as(module, ie.getAnImportedModuleName()))
    or
    exists(ImportMember im | penultimate_module_imported_as(module, im.getImportedModuleName()))
}
...
private predicate penultimate_sys_version_info_index(SubscriptNode s) {
    penultimate_points_to(s.getValue(), theSysVersionInfoTuple(), _, _) and
...
predicate penultimate_scope_precedes(Scope pre, Scope post) {
    not exists(FunctionObject callee, Scope caller |
        callee = layer0_get_a_callee(caller) |
...
/*
DefinitionNode penultimate_definition_of_global_reaches_start_bb(GlobalVariable v, BasicBlock b)  {
    not exists(simple_global_defn(v)) and (
      result = penultimate_definition_of_global_reaches_end_bb(v, b.getAPredecessor())
      or
      not exists(b.getAPredecessor()) and 
      exists(Scope pre |
          layer0_scope_precedes(pre, b.getScope()) and
          result = penultimate_definition_of_global_reaches_end_scope(v, pre)
      )
      or
      not exists(b.getAPredecessor()) and 
      exists(BasicBlock defblock, int i |
          ...
          exists(int j | j < i |
              result = penultimate_definition_of_global_in_bb(v, defblock, j)
          )
          or
          not exists(int j | j < i | exists(layer0_definition_of_global_in_bb(v, defblock, j)))
          and result = penultimate_definition_of_global_reaches_start_bb(v, defblock)
      )
    )
}

DefinitionNode penultimate_definition_of_global_reaches_end_bb(GlobalVariable v, BasicBlock b)  {
    result = penultimate_definition_of_global_reaches_start_bb(v, b) and not exists(layer0_definition_of_global_in_bb(v, b, _))
    or
    exists(int i |
        result = penultimate_definition_of_global_in_bb(v, b, i) and
        not exists(int j | exists(layer0_definition_of_global_in_bb(v, b, j)) and j > i)
    )
}

DefinitionNode penultimate_definition_of_global_reaches_end_scope(GlobalVariable v, Scope s)  {
    result = penultimate_definition_of_global_reaches_end_bb(v, s.getANormalExit().getBasicBlock())
}

pragma [nomagic]
DefinitionNode penultimate_definition_of_global_in_bb(GlobalVariable v, BasicBlock b, int i)  {
    not exists(simple_global_defn(v)) and 
    result.(NameNode).defines(v) and 
    b.getNode(i) = result
    or
    exists(FunctionObject callee |
        b.getNode(i) = layer0_get_a_call(callee) and
        result = penultimate_definition_of_global_reaches_end_scope(v, callee.getFunction())
    )
}

DefinitionNode penultimate_relevant_definition_of_global_in_bb(NameNode n) {
    n.isGlobal() and
    exists(GlobalVariable var  |
        n.uses(var) |
        exists(BasicBlock b, int i, int j |
            i < j and
            result = penultimate_definition_of_global_in_bb(var, b, i) and
            b.getNode(j) = n
        )
    )
}

DefinitionNode penultimate_relevant_definition_of_global(NameNode n) {
    n.isGlobal() and
    exists(GlobalVariable var  |
        n.uses(var) |
        result = penultimate_relevant_definition_of_global_in_bb(n)
        or
        not exists(penultimate_relevant_definition_of_global_in_bb(n)) and
        result = penultimate_definition_of_global_reaches_start_bb(var, n.getBasicBlock())
        or
...
predicate penultimate_global_phi_definition(GlobalVariable v, BasicBlock phi) {
    exists(BasicBlock x, ControlFlowNode defnode | 
        ...
        x.dominanceFrontier(phi) and penultimate_global_ssa_defn(v, defnode)
    )
}
...
predicate penultimate_global_prev_scope(GlobalVariable v, ControlFlowNode node, Scope pre) {
    exists(Scope post |
        penultimate_scope_precedes(pre, post) and
        post.getEntryNode() = node and
        exists(penultimate_get_global_ssa_defn_at_scope_end(v, pre)) and
        penultimate_scope_uses_global(v, post)
    )
}

private predicate penultimate_global_enclosing_scope(GlobalVariable v, ControlFlowNode entry, ControlFlowNode caller) {
    exists(ControlFlowNode defn |
        penultimate_global_ssa_defn(v, defn) and
...
predicate penultimate_global_ssa_defn(GlobalVariable v, ControlFlowNode node) {
    penultimate_global_builtin_definition(v, node, _, _)
    or
    penultimate_global_phi_definition(v, node)
    or
    node.(NameNode).defines(v)
    or
    node.(NameNode).deletes(v)
    or
    penultimate_global_callsite_defn(v, node, _)
    or
    penultimate_global_prev_scope(v, node, _)
    or
    penultimate_global_enclosing_scope(v, node, _)
}

private predicate penultimate_global_callsite_defn(GlobalVariable v, CallNode call, Function f) {
    penultimate_scope_defines_global(v, f) and
    exists(PyFunctionObject fo |
        f = fo.getFunction() and
        call = layer0_get_a_call(fo)
    )
}

pragma[noopt]
private ControlFlowNode penultimate_get_global_ssa_defn_candidate(GlobalVariable v, NameNode use) {
    use.uses(v) and penultimate_global_ssa_defn(v, result) and
...
ControlFlowNode penultimate_get_global_ssa_defn_at_scope_end_candidate(GlobalVariable v, Scope f) {
    penultimate_global_ssa_defn(v, result) and
...
private 
ControlFlowNode penultimate_get_defn_for_call_candidate(GlobalVariable v, ControlFlowNode call) {
     penultimate_global_enclosing_scope(v, _, call) and
     penultimate_global_ssa_defn(v, result) and
     result.strictlyDominates(call)
}

private 
ControlFlowNode penultimate_get_defn_for_call(GlobalVariable v, ControlFlowNode call) {
    result = penultimate_get_defn_for_call_candidate(v, call) and
    not exists(ControlFlowNode better |
        better = penultimate_get_defn_for_call_candidate(v, call) and
        result.strictlyDominates(better)
    )
}

private 
predicate penultimate_global_inner_scope_ssa_points_to(GlobalVariable v, ControlFlowNode defn, Object value, ClassObject cls, ControlFlowNode origin) {
    exists(ControlFlowNode call, ControlFlowNode outer_defn |
        outer_defn = penultimate_get_defn_for_call(v, call) and 
        penultimate_global_enclosing_scope(v, defn, call) and
        penultimate_global_ssa_points_to(v, outer_defn, value, cls, origin)
    )
}

/** Do not use -- testing only */ 
pragma [nomagic]
predicate penultimate_global_ssa_points_to(GlobalVariable v, ControlFlowNode defn, Object value, ClassObject cls, ControlFlowNode origin) {
    penultimate_global_builtin_definition(v, defn, value, cls) and origin = defn
    or
    exists(Function f, ControlFlowNode d |
        penultimate_global_callsite_defn(v, defn, f) and 
        d = penultimate_get_global_ssa_defn_at_scope_end(v, f) and
        penultimate_global_ssa_points_to(v, d, value, cls, origin)
    )
    or
    defn.(NameNode).defines(v) and penultimate_points_to(defn, value, cls, origin)
    or
    penultimate_global_ssa_phi_points_to(v, defn, value, cls, origin)
    or
    exists(Scope pre, ControlFlowNode def |
        ...
        penultimate_global_ssa_points_to(v, def, value, cls, origin)
    )
    or
    penultimate_global_inner_scope_ssa_points_to(v, defn, value, cls, origin)
}
...
predicate penultimate_global_ssa_phi_points_to(GlobalVariable v, BasicBlock phi, Object value, ClassObject cls, ControlFlowNode origin) {
    ...
    exists(ControlFlowNode incoming_defn, BasicBlock pred |
        ...
        penultimate_global_ssa_points_to(v, incoming_defn, value, cls, origin)
    )
}
/** INTERNAL -- Do not not use */
pragma [inline]
...
private predicate defn_for_phi(ControlFlowNode defn, ControlFlowNode phi, GlobalVariable v) {
    penultimate_global_phi_definition(v, phi) and
    penultimate_global_ssa_defn(v, defn)
}
...
predicate penultimate_global_use_points_to(NameNode use, Object value, ClassObject cls, ControlFlowNode origin) {
    exists(GlobalVariable v, ControlFlowNode defn, ControlFlowNode ssa_origin |
        ...
        penultimate_global_ssa_points_to(v, defn, value, cls, ssa_origin) 
        |
