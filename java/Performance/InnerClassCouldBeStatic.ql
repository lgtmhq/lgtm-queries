...
predicate potentiallyStatic(InnerClass c) {
  ...
  forall(InnerClass other | // If nested and non-static, ...
    ...
    (other = c.getASupertype().getSourceDeclaration() and other.fromSource()) or
  ...
  forall(InnerClass nested | nested.getEnclosingType+() = c |
    forall(InnerClass superOfNested | superOfNested = nested.getASupertype+().getSourceDeclaration() |
