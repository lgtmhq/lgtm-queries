...
predicate referencedInXmlFile(Field f) {
	exists(XMLAttribute xmlaField, XMLAttribute xmlaClass |
		xmlaField.getElement().getParent*() = xmlaClass.getElement() |
		xmlaClass.getValue() = f.getDeclaringType().getSourceDeclaration().getQualifiedName() and
		xmlaField.getValue() = f.getName()
	)
}
	...
	Field inferAccessedField() {
		(
			if this.getCallee().hasName("getDeclaredField") then
				...
				(
					// This field must be be public, and be inherited by one of the inferred class types.
					result.isPublic() and
					getInferredClassType().inherits(result)
				)
		) and
