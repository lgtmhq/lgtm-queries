 ...
 */
  ...
  private predicate jumpStep(Node node1, Node node2, boolean preservesValue, Configuration config) {
    jumpStep(node1, node2) and preservesValue = true or
  ...
  private predicate localFlowStep(Node node1, Node node2, boolean preservesValue, Configuration config) {
    localFlowStep(node1, node2) and not config.isBarrierEdge(node1, node2) and preservesValue = true or
   ...
   */
  private predicate useFieldFlow(Configuration config) {
    config.fieldFlowBranchLimit() >= 1
  }
  ...
  private predicate nodeCand1(Node node, boolean stored, Configuration config) {
    ...
    config.isSink(node) and stored = false
    or
   ...
   */
  private predicate simpleArgumentFlowsThrough(ArgumentNode arg, ExprNode call, RefType t, Configuration config) {
   ...
   */
  private predicate flowThroughMethod(Node node1, Node node2, boolean preservesValue, Configuration config) {
    simpleArgumentFlowsThrough(node1, node2, _, config) and preservesValue = false or
   ...
   * through a method.
   */
  private predicate localFlowStepOrFlowThroughMethod(Node node1, Node node2, boolean preservesValue, Configuration config) {
   ...
   */
  private predicate flowOutOfCallable(Node node1, Node node2, boolean allowsFieldFlow, Configuration config) {
    flowOutOfCallable(node1, node2, config) and
    exists(int b, int j |
      b = branch(node1, config) and
      j = join(node2, config) and
      if b.minimum(j) <= config.fieldFlowBranchLimit() then allowsFieldFlow = true else allowsFieldFlow = false
    )
  }
   ...
   */
  private predicate flowIntoCallable(Node node1, Node node2, boolean allowsFieldFlow, Configuration config) {
  ...
  private predicate nodeCandFwd2(Node node, boolean fromArg, boolean stored, Configuration config) {
    nodeCand1(node, false, config) and
    config.isSource(node) and fromArg = false and stored = false
    or
  ...
  private predicate nodeCand2(Node node, boolean toReturn, boolean stored, Configuration config) {
    ...
    config.isSink(node) and toReturn = false and stored = false
    or
  ...
  private predicate readCand(Content f, Configuration conf) {
    readCand2(f, conf)
  }
  ...
  private predicate localFlowExit(Node node, Configuration config) {
    ...
    node.asExpr() instanceof CastExpr or
    config.isSink(node)
  }
   ...
   */
  private predicate localFlowStepPlus(Node node1, Node node2, boolean preservesValue, Configuration config) {
   ...
   */
  pragma[noinline]
  private predicate localFlowBigStep(Node node1, Node node2, boolean preservesValue, Configuration config) {
  ...
  private class FlowContainer extends TFlowContainer {
    string toString() {
      exists(Content f | this = TRegularContent(f) and result = f.toString()) or
      exists(RefType t | this = TSelfRefContent(t) and result = t.toString())
    }
    predicate usesContent(Content f) {
      this = TRegularContent(f) or
      exists(RefType t | this = TSelfRefContent(t) and selfRef(f, t))
    }
    RefType getContainerType() {
      exists(Content f | this = TRegularContent(f) and result = f.getDeclaringType()) or
      this = TSelfRefContent(result)
    }
  }

  private newtype TAccessPathFront = TFrontNil(Type t) or TFrontHead(FlowContainer f)
  ...
  private class AccessPathFront extends TAccessPathFront {
    string toString() {
      exists(Type t | this = TFrontNil(t) | result = t.toString()) or
      exists(FlowContainer f | this = TFrontHead(f) | result = f.toString())
    }
    Type getType() {
      this = TFrontNil(result) or
  ...
  private predicate flowCandFwd(Node node, boolean fromArg, AccessPathFront apf, Configuration config) {
    ...
    if node instanceof CastingNode then
      compatibleTypes(node.getType(), apf.getType())
    else
      any()
  }
  private predicate flowCandFwd0(Node node, boolean fromArg, AccessPathFront apf, Configuration config) {
    ...
    config.isSource(node) and fromArg = false and apf = TFrontNil(getErasedRepr(node.getType()))
    or
    ...
    (
      ...
      exists(Node mid, boolean preservesValue, AccessPathFront apf0 |
        ...
        (preservesValue = true and apf = apf0 or preservesValue = false and apf0 instanceof AccessPathFrontNil and apf = TFrontNil(getErasedRepr(node.getType())))
      )
    )
    or
    exists(Node mid, Content f |
  ...
  private predicate flowCand0(Node node, boolean toReturn, AccessPathFront apf, Configuration config) {
    ...
    config.isSink(node) and toReturn = false and apf instanceof AccessPathFrontNil
    or
    (
      exists(Node mid |
      ...
      exists(Node mid, boolean preservesValue, AccessPathFront apf0 |
        ...
        flowCand(mid, toReturn, apf0, config) and
        (preservesValue = true and apf = apf0 or preservesValue = false and apf0 instanceof AccessPathFrontNil and apf instanceof AccessPathFrontNil and flowCandFwd(node, _, apf, config))
      )
      or
      exists(Node mid, Content f, AccessPathFront apf0 |
        store(node, f, mid) and
  ...
  private newtype TAccessPath = TNil(Type t) or TCons(FlowContainer f, int len) { len in [1..5] }
  ...
  private class AccessPathCons extends AccessPath, TCons {
    override string toString() {
      exists(FlowContainer f, int len | this = TCons(f, len) | result = f.toString() + ", ... (" + len.toString() + ")")
    }
    override AccessPathFront getFront() { exists(FlowContainer f | this = TCons(f, _) | result = TFrontHead(f)) }
  }
   ...
   */
  private predicate flowFwd(Node node, boolean fromArg, AccessPathFront apf, AccessPath ap, Configuration config) {
    flowFwd0(node, fromArg, apf, ap, config) and
    flowCand(node, _, apf, config)
  }
  private predicate flowFwd0(Node node, boolean fromArg, AccessPathFront apf, AccessPath ap, Configuration config) {
    flowCand(node, _, _, config) and
    config.isSource(node) and fromArg = false and ap = TNil(getErasedRepr(node.getType())) and apf = ap.(AccessPathNil).getFront()
    or
    ...
    (
      ...
      exists(Node mid, boolean preservesValue, AccessPathFront apf0, AccessPath ap0 |
        ...
        flowThroughMethod(mid, node, preservesValue, config) and
        (preservesValue = true and ap = ap0 and apf = apf0 or preservesValue = false and ap0 instanceof AccessPathNil and ap = TNil(getErasedRepr(node.getType())) and apf = ap.(AccessPathNil).getFront())
      )
    )
    or
  ...
  pragma[nomagic]
  private predicate flowFwdStore(Node node, Content f, AccessPath ap0, AccessPathFront apf, boolean fromArg, Configuration config) {
    exists(Node mid, AccessPathFront apf0 |
      flowFwd(mid, fromArg, apf0, ap0, config) and
      flowFwdStoreAux(mid, f, node, apf0, apf, config)
    )
  }
  private predicate flowFwdStoreAux(Node mid, Content f, Node node, AccessPathFront apf0, AccessPathFront apf, Configuration config) {
    store(mid, f, node) and
  ...
  pragma[nomagic]
  private predicate flowFwdRead(Node node, Content f, AccessPath ap0, boolean fromArg, Configuration config) {
    exists(Node mid, AccessPathFront apf0 |
      flowFwd(mid, fromArg, apf0, ap0, config) and
      read(mid, f, node) and
      apf0.headUsesContent(f) and
  ...
  private predicate flow0(Node node, boolean toReturn, AccessPath ap, Configuration config) {
    ...
    config.isSink(node) and toReturn = false and ap instanceof AccessPathNil
    or
    (
      exists(Node mid |
        localFlowBigStep(node, mid, true, config) and
      ...
      exists(Node mid, boolean preservesValue, AccessPath ap0 |
        flowThroughMethod(node, mid, preservesValue, config) and
        flow(mid, toReturn, ap0, config) and
        (preservesValue = true and ap = ap0 or preservesValue = false and ap0 instanceof AccessPathNil and ap instanceof AccessPathNil and flowFwd(node, _, _, ap, config))
      )
      or
      exists(Content f, AccessPath ap0 |
  ...
  pragma[nomagic]
  private predicate flowStore(Node node, Content f, boolean toReturn, AccessPath ap0, Configuration config) {
    exists(Node mid |
      store(node, f, mid) and
  ...
  pragma[nomagic]
  private predicate flowRead(Node node, Content f, boolean toReturn, AccessPath ap0, Configuration config) {
    exists(Node mid |
      read(node, f, mid) and
      flow(mid, toReturn, ap0, config)
    )
  ...
  private predicate flow(Node n, Configuration config) {
    flow(n, _, _, config)
  }
  ...
  private newtype TPathNode =
    ...
    TPathNodeSink(Node node, Configuration config) { // The AccessPath on a sink is empty.
  ...
  abstract class PathNode extends TPathNode {
    ...
    private string ppAp() {
      this instanceof PathNodeSink and result = "" or
  ...
  pragma[noinline]
  private predicate contentStoreStep(PathNodeMid mid, Node node, AccessPath ap0, Content f, CallContext cc) {
    ap0 = mid.getAp() and
  ...
  private predicate flowOutOfArgument(PathNodeMid mid, PostUpdateNode node, CallContext cc) {
    exists(PostUpdateNode n, ParameterNode p, Callable callable, CallContext innercc, int i, Call call, ArgumentNode arg |
      mid.getNode() = n and
      ...
      |
      if reducedViableImplInReturn(callable, call) then cc = TReturn(callable, call) else cc = TAnyCallContext()
    )
  }
  ...
  private predicate flowIntoArg(PathNodeMid mid, int i, CallContext cc, Call call, boolean emptyAp) {
    exists(ArgumentNode arg, AccessPath ap |
      ...
      ap = mid.getAp()
      |
      ap instanceof AccessPathNil and emptyAp = true or
  ...
  pragma[nomagic]
  private predicate flowIntoCallable0(PathNodeMid mid, Callable callable, int i, CallContext outercc, Call call, boolean emptyAp) {
  ...
  private predicate flowIntoCallable(PathNodeMid mid, ParameterNode p, CallContext outercc, CallContextCall innercc, Call call) {
    exists(int i, Callable callable, boolean emptyAp |
      ...
      p.isParameterOf(callable, i)
      |
      if reducedViableImplInCallContext(_, callable, call) then innercc = TSpecificCall(call, i, emptyAp) else innercc = TSomeCall(p, emptyAp)
    )
  }
