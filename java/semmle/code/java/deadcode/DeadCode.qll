...
predicate isLive(Callable c) {
  exists(EntryPoint e | c = e.getALiveCallable()) or
...
Callable possibleLivenessCause(Callable c, string reason) {
  (c.(Method).overridesOrInstantiates((Method)result) and reason = "is overridden or instantiated by") or
  (result.calls(c) and reason = "calls") or
  (result.callsConstructor((Constructor)c) and reason = "calls constructor") or
  exists(ClassInstanceExpr e | e.getEnclosingCallable() = result |
    e.getConstructor() = c and reason = "constructs"
  ) or
  (c = result.getSourceDeclaration() and c != result and reason = "instantiates") or
  (c.hasName("<clinit>") and reason = "class initialization" and
  ...
  (c.hasName("<obinit>") and reason = "object initialization" and result = c.getDeclaringType().getAConstructor())
}
...
class SuppressedConstructor extends Constructor {
  SuppressedConstructor() {
    ...
    not exists(Call c |
      ...
      not c.getCaller().(Constructor).isDefaultConstructor()) and
    /*
...
class LiveClass extends SourceClassOrInterface {
  LiveClass() {
    exists(Callable c | c.getDeclaringType().getASupertype*().getSourceDeclaration() = this | isLive(c)) or
    ...
    this instanceof AnonymousClass or
    this instanceof WhitelistedLiveClass or
...
class DeadClass extends SourceClassOrInterface {
  DeadClass() {
    not this instanceof LiveClass
  }

  /**
   * Identify all the "dead" roots of this dead class.
   */
  DeadRoot getADeadRoot() {
    result = getADeadRoot(getACallable())
  }
...
class DeadMethod extends Callable {
  DeadMethod() {
     ...
     */
    not (this instanceof SuppressedConstructor and this.getDeclaringType() instanceof NamespaceClass) and
    not (
      this.(Method).isAbstract() and
      exists(Method m | m.overridesOrInstantiates+((Method)this) | isLive(m))
    ) and
    /*
     ...
     * they are "simple".
     */
    not exists(JPAReadField readField |
      this.getDeclaringType() = readField.getDeclaringType() |
...
class RootdefCallable extends Callable {
  ...
  predicate whitelisted() {
    ...
    this instanceof MainMethod or
    // Premain methods must have certain arguments.
    this instanceof PreMainMethod or
    ...
    (
      ...
      not exists(Method m | hasUsefulBody(m) |
        m.overridesOrInstantiates+(this)
      )
    )
  }
}

private pragma[noinline] predicate overrideAccess(Callable c, int i) {
  exists(Method m | m.overridesOrInstantiates+(c) |
    exists(m.getParameter(i).getAnAccess())
  )
}
