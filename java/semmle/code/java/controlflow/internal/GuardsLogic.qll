...
predicate implies_v2(Guard g1, boolean b1, Guard g2, boolean b2) {
  ...
  exists(SsaVariable v, AbstractValue k |
    ...
    conditionalAssign(v, g2, b2.booleanNot(), k) and
 ...
 */
private predicate conditionalAssign(SsaVariable v, Guard guard, boolean branch, AbstractValue val) {
  exists(ConditionalExpr c |
    ...
    |
    branch = true and val.getExpr() = c.getTrueExpr().getProperExpr() or
    branch = false and val.getExpr() = c.getFalseExpr().getProperExpr()
  ) or
  exists(SsaExplicitUpdate upd, SsaPhiNode phi |
    guard.directlyControls(upd.getBasicBlock(), branch) and
    upd.getDefiningExpr().(VariableAssign).getSource().getProperExpr() = val.getExpr() and
    phi = v and
