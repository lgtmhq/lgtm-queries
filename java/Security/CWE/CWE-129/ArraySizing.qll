...
/**
 * Whether an `ArrayIndexOutOfBoundsException` is ever caught.
 */
...
class PointlessLoop extends WhileStmt {
  PointlessLoop() {
    getCondition().(BooleanLiteral).getLiteral() = "true" and
...
class CheckableArrayAccess extends ArrayAccess {
  ...
  /**
   * Whether we believe this indexing expression will throw an `ArrayIndexOutOfBoundsException` due
  ...
  predicate canThrowOutOfBoundsDueToEmptyArray(FlowSource source, ArrayCreationExpr arrayCreation) {
     ...
     */
    defUsePair(this.getArray().(VarAccess).getVariable(), arrayCreation.getEnclosingStmt(), this.getEnclosingStmt()) and
    /*
